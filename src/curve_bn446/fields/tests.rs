...
use crate::curve_bn446::fields::*;

use ark_algebra_test_templates::fields::*;
use ark_ff::{
    BigInteger, BigInteger448, FftField, FftParameters, Field, Fp6Parameters, FpParameters, One,
...
fn test_fq_repr_from() {
    assert_eq!(
        BigInteger448::from(100),
        BigInteger448([100, 0, 0, 0, 0, 0, 0])
    );
}

#[test]
fn test_fq_repr_is_odd() {
    assert!(!BigInteger448::from(0).is_odd());
    assert!(BigInteger448::from(0).is_even());
    assert!(BigInteger448::from(1).is_odd());
    assert!(!BigInteger448::from(1).is_even());
    assert!(!BigInteger448::from(324834872).is_odd());
    assert!(BigInteger448::from(324834872).is_even());
    assert!(BigInteger448::from(324834873).is_odd());
    assert!(!BigInteger448::from(324834873).is_even());
}

#[test]
fn test_fq_repr_is_zero() {
    assert!(BigInteger448::from(0).is_zero());
    assert!(!BigInteger448::from(1).is_zero());
    assert!(!BigInteger448([0, 0, 1, 0, 0, 0, 0]).is_zero());
}

#[test]
fn test_fq_repr_num_bits() {
    let mut a = BigInteger448::from(0);
    assert_eq!(0, a.num_bits());
    a = BigInteger448::from(1);
    for i in 1..449 {
...
#[test]
fn test_fq_num_bits() {
    assert_eq!(FqParameters::MODULUS_BITS, 446);
    assert_eq!(FqParameters::CAPACITY, 445);
}
...
#[test]
fn test_fq_ordering() {
    // BigInteger448's ordering is well-tested, but we still need to make sure the
    // Fq elements aren't being compared in Montgomery form.
    for i in 0..ITERATIONS {
        assert!(
            Fq::from(BigInteger448::from((i + 1) as u64)) > Fq::from(BigInteger448::from(i as u64))
        );
    }
}
...
fn test_fq_legendre() {
    ...
    assert_eq!(
        QuadraticResidue,
        Fq::from(BigInteger448::from(4)).legendre()
    );
    assert_eq!(
        QuadraticNonResidue,
        Fq::from(BigInteger448::from(5)).legendre()
    );
}
